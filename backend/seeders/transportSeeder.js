...
const seedTransport = async () => {
  try {
    ...
    console.log('Connected to MongoDB for transport seeding...');
    ...
    let newBuses = 0, newRoutes = 0, newPickupPoints = 0;

    for (let i = 0; i < routeConfigs.length; i++) {
      const config = routeConfigs[i];
      
      // Check if bus already exists
      let bus = await Bus.findOne({ busNumber: config.bus });
      if (!bus) {
        // 1. Create Bus if not exists
      ...
      let route = await Route.findOne({ bus: bus._id });
      if (!route) {
        // 2. Create Route if not exists
      ...
      // Check if pickup point already exists for this route
      let pickupPoint = await PickupPoint.findOne({ route: route._id });
      if (!pickupPoint) {
        // 3. Create Pickup Point if not exists
    ...
    console.log(`✅ Transport seeded: ${newBuses} new buses, ${newRoutes} new routes, ${newPickupPoints} new pickup points`);
    await mongoose.disconnect();
    return { success: true, newBuses, newRoutes, newPickupPoints };
  } catch (error) {
